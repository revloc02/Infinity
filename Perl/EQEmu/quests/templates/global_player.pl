sub EVENT_ENTERZONE {
  ...
  if($client->GetPVP())
  {
	# so far this works, but I may need a conditional here that exempts PvP zones from executing the following
	$client->Message(15,"You're PvP is on. Turning off now...");
	quest::pvp(off);
  }
  else
  {
	$client->Message(15,"You're PvP was not on.");
  }
}
